...
<#
    ...
    .PARAMETER BackupFileLocation
        String containing the path to a backup file (.BAK) Can be a directory or individual file.
    
    .PARAMETER DataPath
        Optional string containing path of directory to restore the data files (.MDF) to. Omit to restore to system default location.

    .PARAMETER LogPath
        Optional string containing path of directory to restore the log files (.LDF) to. Omit to restore to system default location.
#>
...
function Invoke-DatabaseRestore
{
    ...
    param
    (
        #[Microsoft.SqlServer.Management.Smo.SqlSmoObject]$SqlServerObject,
        [string]$SQLServer,
    ...
    $BackupFiles = Get-ChildItem -Path $BackupFileLocation -Filter "*.BAK"
    
    $DataPath = if(-not($DataPath))
    {
        Write-Debug "No datapath parameter provided. Will use default..."
        if($SqlServerObject.Settings.DefaultFile.Length -gt 0 ) {$SqlServerObject.Settings.DefaultFile } else { $SqlServerObject.Information.MasterDBLogPath }
    }
    $DataPath = $DataPath.TrimEnd("\")
    Write-Debug "DataPath: $DataPath"
    
    $LogPath = if(-not($LogPath))
    {
        Write-Debug "No logpath parameter provided. Will use default..."
        if($SqlServerObject.Settings.DefaultLog.Length -gt 0 ) {$SqlServerObject.Settings.DefaultLog } else { $SqlServerObject.Information.MasterDBLogPath }
    }
    ...
    {
        $SmoBackupDevice = New-Object -TypeName Microsoft.SqlServer.Management.Smo.BackupDeviceItem($File.Fullname, $DeviceType)
        $SmoRestore.Devices.Add($SmoBackupDevice)
        $SmoRestoreDetails = $SmoRestore.ReadBackupHeader($SqlServerObject)
        Try
        {
            $SmoRestoreFiles = $SmoRestore.ReadFileList($SqlServerObject)
        }
        Catch
        {
            Write-Error "An error occurred reading the file list: $_"
        }
        
        foreach($SmoFile in $SmoRestoreFiles)
        {
            $oRestore += [pscustomobject]@{
                DatabaseName = $SmoRestoreDetails.Rows[0]["DatabaseName"]
                LogicalFileName = $SmoFile.LogicalName
                PhysicalFileName = $( if($SmoFile.Type -eq "L") {$LogPath} else {$DataPath} ) + "\" + [System.IO.Path]::GetFileName($SmoFile.PhysicalName)
            }
        }
    }
    
    
    foreach($Item in $oRestore)
    {
        #Create relocate file object so that we can restore the database to a different path
        $SmoRestoreFiles = New-Object -TypeName Microsoft.SqlServer.Management.Smo.RelocateFile
    ...
    {    
        ...
        $CompletedEventHandler = [Microsoft.SqlServer.Management.Common.ServerMessageEventHandler]{ 
            Write-Verbose "Database '$DbName' created successfuly!" 
        }
        $SmoRestore.add_PercentComplete($Percent)
        ...
        # Take most recent backup set if there is more than one
        $SmoRestore.FileNumber = $SmoRestore.ReadBackupHeader($SqlServerObject).Rows.Count
        
        $StopWatch = [system.diagnostics.stopwatch]::StartNew()
        Write-Verbose "Restoring database '$DbName'..."
        Write-Progress -Id 1 -Activity "Restoring $DbName to $ServerName..." -PercentComplete 0 -Status ([System.String]::Format("Progress: {0} %", 0))
	    
        $SmoRestore.sqlrestore($SqlServerObject)
        
        $Elt = $StopWatch.elapsed.TotalMinutes.ToString("0.00")
        
        Write-Progress -Id 1 -Activity "Restoring $DbName to $ServerName" -Status "Complete." -Completed
    
	    Write-Verbose "Restore complete! Elapsed time: $Elt minutes."
        Exit 0
    }
    Catch
    {
